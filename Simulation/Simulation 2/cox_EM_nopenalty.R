cox_EM_nopenalty <- function(data,K){
  X <- data[,-1:-2]
  X <- as.matrix(X)
  n <- dim(X)[1]
  p <- dim(X)[2]
  Time = matrix(data[,1], ncol = 1)
  Delta = matrix(data[,2], ncol = 1)
  surv = cbind(time = Time, status = Delta)
  colnames(surv) = c("time", "status")
  ####check u.init and initial U matrix
  u.init = sample(1:K, size = n, replace = TRUE)
  U0 = matrix(0.001, K, K)
  diag(U0) = 1 - (K-1)*0.001
  U0 = U0[u.init, ]
  if(K==1){
      U0 = matrix(U0,ncol=1)
  }
  #####initial pi                                                                                                        
  prop0 = apply(U0, 2, mean)
  #### initial class label
  class = apply(U0, 1, function(x){
    which.max(x)[1]
  })
  
  require(survival)
  beta0 = lapply(1:K, function(k){
    idx = which(class==k)
    result <- coxph(Surv(time, status) ~ X1 + X2, data = data[idx,]) # X1 + X2 need to modify
    return(as.numeric(result$coefficients))
  })
  
  
  ploglik = NULL
  mixloglik = NULL
  lastploglik = -Inf
  lastmixloglik = -Inf
  convergence = FALSE
  iter = 0
  iter.max = 1000
  abstol = 1e-2
  reltol = 1e-4  
  ##### 
  while(!convergence & iter<iter.max){
    
    ##E-step
    h0 = NULL
    H0 = NULL
    EE = NULL
    PLL = NULL
    allUEXB = NULL
    for (k in 1:K){
      Uk = U0[,k]
      XB = X%*%beta0[[k]]  
      EXB = exp(XB)
      EE = cbind(EE, EXB)
      UEXB = Uk*EXB
      allUEXB = cbind(allUEXB, UEXB)
      aa = sapply(Time, function(x)sum(UEXB[Time>=x]))
      bb = 1/aa
      bb[bb == Inf] = 0
      h00 = Uk*bb*Delta
      h0_smooth  = bh.smooth(Time, bw = IQR(Time)*n^(-1/5),n,h00)
      H00 = sapply(Time, function(x)sum(h00[Time<=x]))
      h0 = cbind(h0, h0_smooth)
      H0 = cbind(H0, H00)
      PLL = c(PLL, sum((Uk*(XB-log(aa)))[Delta==1&aa>0]))
    }
    
    pdf_est = (h0 * EE)^(Delta %*% t(rep(1, K))) * exp(-H0 * EE)
    CC = pdf_est %*% diag(as.vector(prop0), K)
    
    
    nowploglik = sum(PLL) + sum((U0%*%diag(log(prop0), K))[U0 != 0])
    nowmixloglik = sum(log(apply(CC, 1, sum)))
    ploglik = c(ploglik, nowploglik)
    mixloglik = c(mixloglik, nowmixloglik)
    ####### M step
    if(!convergence){
      U0 = do.call(rbind, lapply(1:n,function(i){
        x = CC[i,]
        return(x / sum(x))
      }))
      prop0 = colMeans(U0)
      
      # weighted log partial likelihood function
      beta0 = lapply(1:K, function(k){
        Uk <- U0[,k]
        log.parlik <- function(beta){
          XB = X%*%beta  
          EXB = exp(XB)
          UEXB = Uk*EXB
          aa = sapply(Time, function(x)sum(UEXB[Time>=x]))
          bb = 1/aa
          bb[bb == Inf] = 0
          PLL = sum((Uk*(XB-log(aa)))[Delta==1&aa>0])
          return(-PLL)
        }
        result <- optim(c(0,0),log.parlik, control = list(maxit = 1000))$par
        return(as.numeric(result))
      }) 
      
      if((abs(nowploglik-lastploglik) < abstol |
          abs(nowploglik-lastploglik) <
          abs(reltol*(nowploglik + reltol))))
        convergence = TRUE
      
      lastploglik = nowploglik
      lastmixloglik = nowmixloglik
      iter = iter + 1
      
      ######
      if(iter==iter.max)
        warning("EM iterations reach iter.max!")
      class = apply(U0, 1, function(x){
        which.max(x)[1]
      })
      finalmixloglik = mixloglik[length(mixloglik)]
      finalploglik = ploglik[length(ploglik)]
      BIC = 2*finalmixloglik - (K-1 + K*p)*log(n)
      print(prop0)
      print(beta0)
      print(class)
      print(iter)
    }
  }
  
  
  result = list(U = U0,
                k = K,
                CC = CC,
                pi = prop0,
                beta = beta0,
                BIC = BIC,
                class = class,
                finalmixloglik = finalmixloglik,
                mixloglik = mixloglik,
                iter = iter,
                convergence = iter < iter.max)
  return(result)
}



 